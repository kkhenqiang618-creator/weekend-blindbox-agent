...
function normalizePoi(raw) {
  const id = asString2(raw.id, "unknown_poi");
  const type = asString2(raw.type, "\u4F11\u95F2\u5A31\u4E50");
  const limits = asStringArray2(raw.limits);
  return {
    id,
    name: asString2(raw.name, id),
    lat: optionalNumber(raw.lat),
    lng: optionalNumber(raw.lng),
    type,
    subType: asString2(raw.subType, type),
    address: optionalString(raw.address),
    area: optionalString(raw.area),
    businessDistrict: asString2(raw.businessDistrict, "\u672A\u77E5\u5546\u5708"),
    ...
    reason: asString2(raw.reason, "\u9002\u5408\u52A0\u5165\u672C\u6B21\u5468\u672B\u8DEF\u7EBF"),
    blindBoxThemes: asStringArray2(raw.blindBoxThemes),
    availableTools: asStringArray2(raw.availableTools),
    ...
    replaceableBy: asStringArray2(raw.replaceableBy),
...
function normalizeFitPeople(value) {
  const rawValues = asStringArray2(value);
...
async function handler(req, res) {
  ...
  try {
    ...
    const userProfile = sessionId ? await learnUserProfile(sessionId) : void 0;
    const userInput = {
