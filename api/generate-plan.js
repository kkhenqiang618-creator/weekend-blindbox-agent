...
async function reserveOrJoinPlan(route) {
  return route.steps.map((step) => ({
    ...
    status: "success",
    poiId: step.poi.id,
    message: step.poi.bookingRequired ? `${step.poi.name} \u5DF2\u5B8C\u6210\u9884\u7EA6\u6A21\u62DF` : `${step.poi.name} \u5DF2\u52A0\u5165\u884C\u7A0B`,
    result: {
      joined: true,
...
async function generateWeekendPlan(body) {
  const { generatePlan: generatePlan2, pois: pois2 } = await loadAgent();
  const userInput = {
  ...
  return generatePlan2(userInput, { pois: pois2 });
}
...
async function handler(req, res) {
  ...
  try {
    const plan = await generateWeekendPlan(req.body ?? {});
